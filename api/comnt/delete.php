...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'DELETE':
        ...
        $commentId = intval($data['comment_id'] ?? ($_GET['comment_id'] ?? 0));
        if ($commentId <= 0) {
            http_response_code(400);
        ...
        if (intval($row['user_id']) !== $userId) {
            ...
            echo json_encode(array("success" => false, "message" => "You do not have permission to delete this comment."));
            exit;
        }

        $del = $conn->prepare("DELETE FROM comment WHERE comment_id = ?");
